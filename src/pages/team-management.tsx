          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
              <div>
                ...
                <p className="text-slate-500">Manage your team and their tasks</p>
              </div>
            ...
            {/* Team Members */}
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-slate-900 mb-4">Your Team</h3>
                    ...
                    <Card key={member.id} className="overflow-hidden hover:shadow-md transition-shadow">
                      <CardHeader className="pb-2">
                        <div className="flex items-center space-x-3">
                          <AvatarInitials name={member.name} className="h-10 w-10 text-sm" />
                          <div>
                      ...
                      <CardContent>
                        <div className="text-sm">
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-slate-500">Task Summary:</span>
                          </div>
                          ...
                          <div className="grid grid-cols-3 gap-2 text-center">
                            <div className="bg-indigo-50 rounded p-2">
                              ...
                              <p className="font-semibold text-indigo-600">
                                {getTeamMemberTasks(member.id).filter((t: any) => t.status === 'pending').length}
                              </p>
                            </div>
                            <div className="bg-amber-50 rounded p-2">
                              <p className="text-xs text-slate-500">In Progress</p>
                              <p className="font-semibold text-amber-600">
                                {getTeamMemberTasks(member.id).filter((t: any) => t.status === 'in-progress').length}
                              </p>
                            </div>
                            <div className="bg-emerald-50 rounded p-2">
                              <p className="text-xs text-slate-500">Completed</p>
                              <p className="font-semibold text-emerald-600">
                                {getTeamMemberTasks(member.id).filter((t: any) => t.status === 'completed').length}
                              </p>
                            </div>
                          </div>
            ...
            {teamMembers && teamMembers.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                <div className="p-4 md:p-6 border-b border-slate-200">
                  <h3 className="text-lg font-semibold text-slate-900">Team Tasks</h3>
                </div>
      ...
      <Dialog open={isNewTaskModalOpen} onOpenChange={setIsNewTaskModalOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <TaskForm 
            ...
            task={selectedTeamMember ? { assigneeId: selectedTeamMember } : undefined}
          />
        </DialogContent>
      </Dialog>
    </div>
