...
const TaskList = () => {
  ...
  // Stats for the cards
  const taskStats = {
    pending: tasks ? tasks.filter(task => task.status === TaskStatus.PENDING).length : 0,
    inProgress: tasks ? tasks.filter(task => task.status === TaskStatus.IN_PROGRESS).length : 0,
    completed: tasks ? tasks.filter(task => task.status === TaskStatus.COMPLETED).length : 0,
  };
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Status / Priority</th>
          ...
          {sortedTasks.length > 0 && (
            <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between">
              <div className="text-sm text-slate-700">
                Showing <span className="font-medium">1</span> to <span className="font-medium">{sortedTasks.length}</span> of <span className="font-medium">{tasks?.length}</span> results
              </div>
